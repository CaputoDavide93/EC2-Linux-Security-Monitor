...
remove_user_aliases() {
    ...
    # A sed range whose end never matches deletes to end of file, so pick the
    # end pattern by format rather than running both ranges blindly.
    if grep -qx "# End security monitoring shortcuts" "$bashrc" 2>/dev/null; then
    ...
    elif grep -q "^security-health()" "$bashrc" 2>/dev/null; then
